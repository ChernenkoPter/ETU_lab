...
def optimal_partition(words_to_counts, is_linear):
    if is_linear:  # find groups for optimal partitions
        group_sizes = find_optimal_linear_partition_group_sizes(words_to_counts, [1, len(words_to_counts)])
    else:
        group_sizes = find_optimal_diadic_partition_group_sizes(words_to_counts, [1, len(words_to_counts)])
